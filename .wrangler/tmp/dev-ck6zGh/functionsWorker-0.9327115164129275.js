...
// .wrangler/tmp/pages-bw0EDJ/functionsWorker-0.9327115164129275.mjs
...
var USER_AGENT = "AndesRadar/1.0 (+public-job-search)";
var SOURCES = {
...
function jsonLdBlocks(html) {
  return [...html.matchAll(/<script[^>]*type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)].flatMap((match2) => {
    try {
...
__name(jsonLdBlocks, "jsonLdBlocks");
__name2(jsonLdBlocks, "jsonLdBlocks");
...
__name(normalizePosting, "normalizePosting");
__name2(normalizePosting, "normalizePosting");
...
function extractLinks(html, source) {
  const links = [...html.matchAll(/href=["']([^"'#?]+)["']/gi)].map((match2) => match2[1]).filter((path) => source.pattern.test(path));
  return [...new Set(links)].slice(0, 20).map((path) => new URL(path, source.origin).href);
}
__name(extractLinks, "extractLinks");
__name2(extractLinks, "extractLinks");
...
__name(titleFromUrl, "titleFromUrl");
__name2(titleFromUrl, "titleFromUrl");
...
__name2(fetchSource, "fetchSource");
async function fetchComputrabajo(query) {
  ...
  const links = [...new Set([...html.matchAll(/href=["'](\/ofertas-de-trabajo\/oferta-de-trabajo-[^"'#?]+)["']/gi)].map((match2) => `${source.origin}${match2[1]}`))].slice(0, 10);
  const jobs = [];
...
__name(fetchComputrabajo, "fetchComputrabajo");
__name2(fetchComputrabajo, "fetchComputrabajo");
async function onRequestGet(context2) {
  const params = new URL(context2.request.url).searchParams;
  const query = params.get("q")?.trim() || "empleo";
  const requested = params.get("source") || "all";
  if (query.length > 160) return Response.json({ error: "La b\xFAsqueda es demasiado larga." }, { status: 400 });
  ...
  return Response.json({ query, jobs, errors, fetchedAt: (/* @__PURE__ */ new Date()).toISOString() }, { headers: { "Cache-Control": "public, max-age=300, s-maxage=900" } });
}
...
// .wrangler/tmp/bundle-KsL3si/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-KsL3si/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9327115164129275.js.map
